...
| M2-3 | One safe real-provider adapter | M2-2 | product | DONE (`0771c10`, verified) — Cline adapter (`internal/provider/cline`), model `cline-pass/deepseek-v4.1-flash` (the `cline-pass/` prefix routes to the ClinePass subscription; the bare `deepseek/` id bills metered pay-as-you-go — verified live); unit tests network-free; frozen interface intact |
| M2-4 | Four-arm review benchmark | M2-3 | validation | DONE — M2-4a harness+scoring `87960d2` (fake-only $0); M2-4b live run complete (`run-20260922T025113Z.json` + `comparison-20260922T025113Z.md`, ~$1.04 billed to ClinePass subscription). Result: all 4 arms hit recall 1.00 / 0 important misses on both fixtures; they differ on false findings (structured lowest 0.67–1.00, roles highest 2.33–5.67) and cost/latency (single-call arms ~30–40s ~$0.015; 4-call roles/generic ~2–2.5min ~$0.06–0.08). Signal: structured is best value; roles' extra 3 calls add noise+cost without recall gain on this data |
